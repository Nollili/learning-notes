...
1. **JavaScript** – Execute event handlers, animation logic, React rendering, etc.  
2. **Style Calculation** – Compute final CSS styles for all elements.  
3. **Layout (Reflow)** – Determine element sizes and positions.  
4. **Paint** – Fill pixels (colors, shadows, images, borders, text).  
5. **Composite** – Combine painted layers into the final frame.

If any step exceeds **16ms**, you drop below 60 FPS → visible **jank**.
